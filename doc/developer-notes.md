...
The Aurora Borealis Core Doxygen documentation can be generated by running `make docs` from the top-level
...
Aurora Borealis Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 17333.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 17332 for connections and services them.
...
However, in open source software such as Aurora Borealis Core, where everyone uses
...
pay attention to for reviewers of Aurora Borealis Core code.

General Aurora Borealis Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Aurora Borealis Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream; compatibility-related bug fixes may also be appropriate to report to the original Ravencoin upstream project
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `auroraborealis-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `auroraborealis-cli` and the GUI debug console use this table to determine how to
