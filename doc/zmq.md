...
The Aurora Borealis Core daemon can be configured to act as a trusted "border
...
The ZeroMQ feature in Aurora Borealis Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building auroraborealisd:
...
For instance:
    ...
    $ auroraborealisd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/auroraborealisd.tx.raw
...
These options can also be provided in auroraborealis.conf.
...
From the perspective of auroraborealisd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into auroraborealisd directly. Furthermore, no information is
...
using. auroraborealisd appends an up-counting sequence number to each
