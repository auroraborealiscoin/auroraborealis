...
const QString RAVEN_IPC_PREFIX("auroraborealis:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/auroraborealis-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/auroraborealis-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/auroraborealis-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the auroraborealis: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(RAVEN_IPC_PREFIX, Qt::CaseInsensitive)) // auroraborealis: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click auroraborealis: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start auroraborealis: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling auroraborealis: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in auroraborealis: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(RAVEN_IPC_PREFIX, Qt::CaseInsensitive)) // auroraborealis: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseRavenURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Aurora Borealis address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
