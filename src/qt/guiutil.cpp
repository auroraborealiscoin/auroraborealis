...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter an Aurora Borealis address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no auroraborealis: URI
    if(!uri.isValid() || uri.scheme() != QString("auroraborealis"))
        return false;
...
{
    // Convert auroraborealis:// to auroraborealis:
    //
    //    Cannot handle this later, because auroraborealis:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("auroraborealis://", Qt::CaseInsensitive))
    {
        uri.replace(0, 17, "auroraborealis:");
    }
...
{
    QString ret = QString("auroraborealis:%1").arg(info.address);
    int paramCount = 0;
...
bool openRavenConf()
{
    ...
    /* Open auroraborealis.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aurora Borealis.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aurora Borealis (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Aurora Borealis (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Aurora Borealis\n";
        else
            optionFile << strprintf("Name=Aurora Borealis (%s)\n", chain);
